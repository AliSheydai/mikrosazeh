...
import { ArrowUpRight, Linkedin, PartyPopper, Rocket } from "lucide-react";
...
export function HeaderPro() {
  ...
  useEffect(() => {
    ...
    if (random < 0.75) {
      setContent({
        text: "اپ را سریع بسازید",
        productName: "پیش‌الگو میکروسازه",
        link: "#",
      });
    } else {
      setContent({
        text: "کامپوننت‌های جدید را کشف کنید",
        productName: "میکروسازه حرفه‌ای",
        link: "#",
      });
    }
  }, []);
  ...
  return (
    <div className="flex items-center justify-end gap-1 flex-1">
      <div className="flex items-center justify-start md:justify-end gap-1 flex-1">
        ...
        <Link
          href="https://github.com/AliSheydai"
          target="_blank"
        ...
        <Link
          href="https://www.linkedin.com/in/ali-sheydai-a63a472aa?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app"
          target="_blank"
          className="hidden group relative md:inline-flex items-center gap-2 px-1.5 py-1.5 text-sm rounded-lg bg-zinc-900 dark:bg-zinc-100 transition-colors hover:bg-zinc-800 dark:hover:bg-zinc-200"
        >
          <Linkedin className="w-4 h-4 text-white dark:text-zinc-900" />
        </Link>
      </div>
    </div>
  );
