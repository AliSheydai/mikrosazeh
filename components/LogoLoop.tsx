...
// هوک useAnimationLoop را با این نسخه جایگزین کنید:
const useAnimationLoop = (
  ...
  isVertical: boolean,
  direction: string // پارامتر جهت را اضافه کنید
) => {
  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      // آپدیت آفست به صورت همیشه مثبت برای جلوگیری از پرش
      offsetRef.current += Math.abs(velocityRef.current) * deltaTime;
      offsetRef.current = offsetRef.current % seqSize;

      // محاسبه ترنسفورم بر اساس جهت
      let finalOffset = offsetRef.current;
      if (isReverse) {
        // برای حرکت به راست/پایین، آفست را معکوس می‌کنیم تا آیتم‌ها وارد شوند
        finalOffset = seqSize - offsetRef.current;
      }

      const transformValue = isVertical
        ? `translate3d(0, ${-finalOffset}px, 0)`
        : `translate3d(${-finalOffset}px, 0, 0)`;
  ...
  }, [targetVelocity, seqWidth, seqHeight, isHovered, hoverSpeed, isVertical, direction]);
};
